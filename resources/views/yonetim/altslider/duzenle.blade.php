...
@section('content')
    <form action="{{ route('yonetim.altslider.duzenle', $gelen->id) }}" method="post" enctype="multipart/form-data">
        {{ csrf_field() }}
                ...
                <div class="md-card-content">
                    <div class="uk-grid" data-uk-grid-margin>
                        <div class="uk-width-medium-1-1">
                            <h4 class="heading-ext">Alt Slider Düzenle</h4>
                            <div class="uk-form-row">
                                <label>Slider Adı</label>
                                <input value="{{ $gelen->name }}" type="text" class="md-input" name="name"/>
                            </div>
                            <div class="uk-form-row">
                            ...
                            <div class="uk-form-row">
                                <div class="md-card">
                                    <div class="md-card-content">
                                        ...
                                        <div style="max-height: 200px" id="result-file"
                                             class="md-card-content"><img src="{{ config('app.url').'images/'.$gelen->img }}" style="max-height: 200px" id="result"/></div>
                                    </div>
                                </div>
                            </div>
                            <div class="md-card-content">
                                ...
                                <div class="uk-grid" data-uk-grid-margin>
                                    <div id="on-switch" class="uk-width-medium-1-3">
                                        <input name="link_ata" {{ $gelen->link == NULL ? '' : 'checked' }} type="checkbox" data-switchery data-switchery-size="large"
                                ...
                                <div id="slider-link" style="margin-top: 30px" class="uk-form-row">
                                    ...
                                    <input onClick="this.setSelectionRange(0, this.value.length)" type="text"
                                           class="md-input" name="linki" value="{{ $gelen->link }}"/>
                                </div>
                            </div>
                            <div style="display: {{ $gelen->link == NULL ? 'block' : 'none' }}" id="slider-content" class="uk-form-row">
                                <label>Slider İçeriği</label>
                                <textarea id="wysiwyg_ckeditor" name="icerik" cols="30" rows="20">{!! $gelen->icerik !!}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
