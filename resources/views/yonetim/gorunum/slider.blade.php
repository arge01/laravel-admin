    ...
    <div id="page_content_inner">
        <div class="md-card">
            <div class="md-card-content">
                <h4 class="heading-ext">Slider/ Görünüm Ayarları</h4>
                <div class="uk-overflow-container uk-margin-bottom">
                    <table class="uk-table uk-table-align-vertical uk-table-nowrap tablesorter tablesorter-altair" id="ts_pager_filter">
                        <thead>
                        <tr>
                            <th style="width: 10%;">Resmi</th>
                            <th>#</th>
                            <th>Adı</th>
                            <th>Linki</th>
                            <th>Ek/ Tarihi</th>
                        ...
                        @foreach($data as $i => $dd)
                            <tr id="tr-{{ $dd->id }}">
                                <td><img src="{{ config('app.url').'images/'.'thump-'.$dd->img }}" alt=""></td>
                                <td>{{ $i++ }}</td>
                                <td>{{ $dd->name }}</td>
                                <td>{{ $dd->slug }}</td>
...
@section('javascript')
    ...
    <script>
        ...
        $('#page_settings_submit').on('click', function () {
            $.ajax({
                type: 'POST',
                url: '{{ route('yonetim.gorunum.slider') }}',
                data: data,
                dataType: 'json',
