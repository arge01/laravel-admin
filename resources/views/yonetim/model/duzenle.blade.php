...
@section('content')
    <form action="{{ route('yonetim.model.duzenle', $gelen->id) }}" method="post" enctype="multipart/form-data">
        {{ csrf_field() }}
                ...
                <div class="md-card-content">
                    <div class="uk-grid" data-uk-grid-margin>
                        <div class="uk-width-medium-1-1">
                            <h4 class="heading-ext">Model Güncelle</h4>
                            ...
                            <div class="uk-form-row">
                                <div class="parsley-row">
                                    <label>Hangi Ürüne</label>
                                    <select disabled id="kategori" name="urun" multiple>
                                        <option selected value="{{ $gelen->projesi->id }}">{!! $gelen->projesi->name !!}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="uk-form-row">
                                <div class="md-card">
                                    <div class="md-card-content">
                                        ...
                                        <div id="result-file" class="md-card-content"><img src="{{ config('app.url').'images/'.$gelen->img }}" id="result" style="max-height: 200px"/></div>
                                    </div>
                                </div>
                            </div>

                            <div class="uk-form-row">
                                <label>Model İçeriği</label>
                                <textarea name="text" id="wysiwyg_ckeditor" cols="30" rows="20">{{ $gelen->text }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
